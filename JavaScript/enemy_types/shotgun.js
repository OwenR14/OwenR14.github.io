class Shotgun extends Enemy{
    constructor(xPos,yPos, damage, health, gunDamage){
        super(xPos,yPos, 45, 80, damage, health)
        this.bulletCooldown = Math.floor(Math.random()*240)+90
    ...
    update(){
        ...
        this.hitboxes[1].yPos = this.position.y + this.height*3/4
            let xAng = 0
            if (player.position.x<this.position.x){c.drawImage(document.getElementById("Shotgun"), this.position.x, this.position.y)
            xAng = 7}
            else{c.drawImage(document.getElementById("ShotgunRight"), this.position.x, this.position.y) 
            xAng = 21}
            c.save()
            ...
            if (player.position.x<this.position.x){c.drawImage(document.getElementById("ShotLeft"), this.position.x-10, this.position.y+20)}
            else{c.drawImage(document.getElementById("ShotRight"), this.position.x-15, this.position.y+20)}
            c.restore()
            if (this.bulletCooldown > 0){
                ...
                this.gunBulletArr = new Array()
                for (let x = 0; x < 10; x++){
                    this.gunBulletArr[x] = (new Bullet(this.position.x+xAng, this.position.y+26, 10, Math.PI-angle + (Math.sin(Math.random()*Math.PI/6)-Math.PI/12), currentRoom, this.gunDamage, 5, "enemy"))
                }
                this.bulletCooldown = 350
            }
            if(this.gunBulletArr != null){for (let y = 0; y < this.gunBulletArr.length; y++){
                this.gunBulletArr[y].update()
            }}
    }
}